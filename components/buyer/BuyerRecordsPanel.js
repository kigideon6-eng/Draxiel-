...
export default function BuyerRecordsPanel({ profile }) {
  ...
  async function handleDeleteProject() {
    if (
      !confirm('Delete this venture? Entries will keep their data but lose the venture link.')
    )
      return;
  ...
  async function handleCreateProjectInline() {
    if (!newProjectName.trim()) {
      setError('Give the venture a name first.');
      return;
    }
  ...
  return (
    <div>
      ...
      <p className="text-sm text-charcoal/70 mb-4">
        Track what you buy and what you resell, per venture.
      </p>
      ...
      <div className="flex items-center gap-2 flex-wrap mb-4">
        ...
        {selectedProject !== 'all' && (
          <button onClick={handleDeleteProject} className="text-xs text-red-700 font-bold">
            Delete this venture
          </button>
        )}
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSaveEntry} className="card p-4 mb-6 space-y-3">
          <div className="bg-gold/10 border border-gold/40 rounded p-3">
            <label className="block text-sm font-bold mb-1">Which venture is this for?</label>
            <select
              value={entryProject}
              onChange={(e) => setEntryProject(e.target.value)}
              className="w-full border border-line rounded px-3 py-2 text-sm bg-white mb-2"
            >
              <option value="">No venture (general record only)</option>
              {projects.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.name}
                </option>
            ...
            {!showNewProjectInline ? (
              <button
                type="button"
                onClick={() => setShowNewProjectInline(true)}
                className="text-xs font-bold text-forest underline"
              >
                + Create a new venture
              </button>
            ) : (
              <div className="space-y-2 mt-2 border-t border-gold/40 pt-2">
                <input
                  value={newProjectName}
                  onChange={(e) => setNewProjectName(e.target.value)}
                  placeholder="Venture name, e.g. Palm oil resale"
                ...
                >
                  {creatingProject ? 'Creating...' : 'Create and use this venture'}
                </button>
              </div>
            )}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-line bg-cream">
                  ...
                  <th className="text-left p-3 hidden sm:table-cell">Venture</th>
