...
export default function OrdersPanel({ profile, viewerRole }) {
  ...
  async function handlePayNow(order) {
    ...
    const amount = Number(order.agreed_price) * Number(order.quantity);
    ...
    try {
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          orderId: order.id,
          amount,
          email: profile.email || `${profile.id}@fairharvest.ai`,
        }),
      });
  ...
  return (
    <div>
      ...
      {payError && <p className="text-sm text-red-700 mb-3">{payError}</p>}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {orders.map((order) => {
            const otherUserId = viewerRole === 'farmer' ? order.buyer_id : order.farmer_id;
            const orderTotal = Number(order.agreed_price) * Number(order.quantity);
            const fee = orderTotal * 0.03;
                  ...
                  <div>
                    ...
                    {viewerRole === 'buyer' && order.escrow_status === 'pending' && (
                      <p className="text-xs text-charcoal/50">
                        Includes 3% platform fee: ₦{fee.toLocaleString()}
                      </p>
                    )}
                  </div>
                  <span
