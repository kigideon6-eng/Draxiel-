...
export default function EquipmentPanel({ profile }) {
  ...
  const hasSentMessage = chatMessages.some((m) => m.sender_id === profile?.id);
  ...
  async function loadMyBookings() {
    const { data } = await supabase
      ...
      .select(
        '*, equipment_listings(item_name, price_unit), owner:profiles!equipment_bookings_owner_id_fkey(full_name, phone)'
      )
  ...
  async function handleStartBooking(listing) {
    ...
    const [a, b] = [profile.id, listing.owner_id].sort();
    ...
    if (!existing) {
      const { data: created, error: createError } = await supabase
        ...
        .select()
        .single();
      if (!createError) existing = created;
    }
  ...
  useEffect(() => {
    if (!convo) return;
    const interval = setInterval(() => loadChatMessages(convo.id), 4000);
  ...
  async function handleSendChat(e) {
    e.preventDefault();
    if (!chatInput.trim() || !convo) return;
    ...
    await supabase.from('notifications').insert({
      ...
      message: 'You have a new message about your equipment listing.',
    });
    setChatInput('');
      ...
      {booking && (
        <div className="fixed inset-0 bg-charcoal/50 flex items-center justify-center p-4 z-50">
          <div className="card p-6 max-w-sm w-full">
            <h3 className="font-bold text-lg mb-1">{booking.item_name}</h3>
            ...
            {chatLoading ? (
              <p className="text-sm text-charcoal/60">Starting chat with owner...</p>
            ) : (
              <>
                <p className="text-xs text-charcoal/60 mb-2">
                  Message the owner about where and when you need this before paying.
                </p>
                <div className="max-h-48 overflow-y-auto space-y-2 mb-3 border border-line rounded p-2">
                  {chatMessages.length === 0 ? (
                    <p className="text-xs text-charcoal/50">
                      No messages yet. Say hello and share your location.
                    </p>
                  ) : (
                    chatMessages.map((m) => (
                      <div
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={bookingLoading || !hasSentMessage}
              >
                {bookingLoading
                  ? 'Starting...'
                  : hasSentMessage
