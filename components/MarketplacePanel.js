...
import InterestsPanel from './buyer/InterestsPanel';
...
export default function MarketplacePanel({ profile, onOrderPlaced }) {
  ...
  function isMatch(listing) {
    if (interestKeywords.length === 0) return false;
    const name = (listing.item_name + ' ' + (listing.description || '')).toLowerCase();
    return interestKeywords.some((keyword) => name.includes(keyword));
  }

  const sortedListings = [...listings].sort((a, b) => {
    const aMatch = isMatch(a) ? 1 : 0;
    const bMatch = isMatch(b) ? 1 : 0;
  ...
  return (
    <div>
      <InterestsPanel profile={profile} onSaved={(val) => setInterests(val)} />
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {sortedListings.map((listing) => (
            <div key={listing.id} className={`card p-4 ${isMatch(listing) ? 'border-gold border-2' : ''}`}>
              {isMatch(listing) && (
                <span className="text-xs font-bold text-gold-dark bg-gold/10 px-2 py-0.5 rounded mb-2 inline-block">
